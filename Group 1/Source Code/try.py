...
class LinkList:
    def __init__(self, root):
        self.window = root
        self.window.config(bg="#ea91a1")

        self.c1 = ("#CCCCFF")
        self.c2 = ("#073763")
        # Make Canvas
        self.canvas_make = Canvas(
            self.window, bg="#00b6cf", width=1350, height=500, relief=RAISED, bd=8)
        ...
        self.pointing_line_temp_left = 195
        self.pointing_line_temp_up = 65

        # temp label coordinate initialization
        self.temp_label_x = 150
        self.temp_label_y = 40

        # temp label corresponding block coordinate initialization

        self.temp_pointer_left = 180
        self.temp_pointer_up = 50
    ...
    def heading_with_label_subheading(self):
        self.head_name = Label(self.window, text="Link List Animation", font=(
            "Arial", 35, "bold", "italic"), bg="#CCCCFF", fg="#073763")
        self.head_name.place(x=350, y=20)

        self.information = Label(self.window, text="Start is a pointer that pointing the first node and temp pointer is used at the time of \ninsert last and delete last to reach to the targeting location", font=(
            "Arial", 20, "bold", "italic"), bg="#CCCCFF", fg="#073763")
    ...
    def make_start_with_other(self):
        ...
        self.start_pointer = self.canvas_make.create_rectangle(
            self.start_left, self.start_up, self.start_left+30, self.start_up+30, fill="black", outline="red", width=3)

        # making of start label
        self.start_label = Label(self.canvas_make, text="Start", font=(
            "Arial", 15, "bold"), bg="#CCCCFF", fg=self.c2)
        self.start_label.place(x=40, y=410)

        # making of start pointing line
        self.pointing_line_start = self.canvas_make.create_line(
            65, 327, 65, 395, width=3, fill="gray")

        # Making of NULL which is primarily pointing by start

        self.start_initial_point_null = Label(self.canvas_make, text="NULL", font=(
            "Arial", 15, "bold"), bg="#CCCCFF", fg="gray")
        ...
        self.temp_label = Label(self.canvas_make, text="Temp", font=(
            "Arial", 15, "bold"), bg="#CCCCFF", fg="#073763")

        self.temp1_label = Label(self.canvas_make, text="temp1", font=(
            "Arial", 15, "bold"), bg="#CCCCFF", fg="#073763")

    # Make some buttons to give instruction to program

    def make_btn(self):
        self.insert_at_beg = Button(self.window, text="Insert First", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, command=lambda: self.make_node_with_label(1))
        self.insert_at_beg.place(x=20, y=540)

        self.insert_at_last = Button(self.window, text="Insert Last", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, command=lambda: self.make_node_with_label(0))
        self.insert_at_last.place(x=160, y=540)

        self.delete_at_first = Button(self.window, text="Delete First", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, command=self.delete_first_node)
        self.delete_at_first.place(x=300, y=540)

        self.delete_at_last = Button(self.window, text="Delete last", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, command=lambda: self.delete_last_node(0))
        self.delete_at_last.place(x=448, y=540)

        self.insert_after_node = Button(self.window, text="Insert after node", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL, command=self.set_of_input_method)
        self.insert_after_node.place(x=590, y=540)

        self.delete_particular_node = Button(self.window, text="Delete particular node", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL, command=self.delete_single_node_infrastructure)
        self.delete_particular_node.place(x=785, y=540)

        self.update = Button(self.window, text="Update", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL)  # command=self.up1)
        self.update.place(x=1030, y=540)

        self.search = Button(self.window, text="Search", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL, command=self.search_A)
        self.search.place(x=1135, y=540)

        self.back = Button(self.window, text="Back", bg="#C8E7F5", fg="black", font=(
            "Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL, command=self.window.quit)  # command=self.delete_single_node_infrastructure)
        self.back.place(x=1240, y=540)
    ...
    def set_of_input_method(self):
        ...
        self.position_label = Label(self.window, text="Enter the node position after you want to insert new node", font=("Arial", 13, "bold"), bg="",
                                    fg="brown")
        self.position_label.place(x=750, y=620)

        self.position_take_entry = Entry(self.window, font=("Arial", 13, "bold"), bg="#CCCCFF", state=NORMAL,
                                         fg="#073763", relief=SUNKEN, bd=5, textvar=self.position_entry)
        ...
        self.find_btn = Button(self.window, text="Find", font=("Arial", 10, "bold"),
                               bg="#CCCCFF", fg="black", relief=RAISED, bd=3, padx=3, pady=3,
    ...
    def checking_of_existence(self):
        ...
        if int(self.position_entry.get()) < 1 or int(self.position_entry.get()) > len(self.node_value_store):
            ...
            self.information.config(
                text="start is a pointer that pointing the first node and temp pointer is used at the time of \ninsert last and delete last to reach to the targeting location")
        else:
    ...
    def make_node_with_label(self, take_notation):
        ...
        # New_node label make
        self.new_node_label = Label(self.canvas_make, text="New node", font=("Arial", 13, "bold"), bg="#CCCCFF",
                                    fg="green")
        self.new_node_label.place(x=30, y=90)

        # Data widget making
        self.data = self.canvas_make.create_rectangle(
            self.data_left, self.data_up, self.data_left+40, self.data_up+30, outline="green", fill="yellow", width=3)

        # Data widget corresponding label making
        self.data_label = Label(self.canvas_make, text="data", font=("Arial", 13, "bold"), bg="#CCCCFF",
                                fg="green")
        self.data_label.place(x=self.data_label_x, y=self.data_label_y)

        # Next widget making
        self.next = self.canvas_make.create_rectangle(
            self.data_left+50, self.data_up, self.data_left+50+40, self.data_up+30, outline="green", fill="yellow", width=3)

        # Next widget corresponding label making
        self.next_label = Label(self.canvas_make, text="next", font=("Arial", 13, "bold"), bg="#CCCCFF",
                                fg="green")
    ...
    def input_take(self, take_notation):
        ...
        self.element_take_label = Label(self.window, text="Enter the element value",
                                        bg="#CCCCFF", fg="brown", font=("Arial", 12, "bold"))
        self.element_take_label.place(x=10, y=620)

        self.element_take_entry = Entry(self.window, font=("Arial", 13, "bold"), bg="#CCCCFF", state=NORMAL,
        ...
        self.add_btn = Button(self.window, text="Add", font=("Arial", 10, "bold"),
                              bg="#CCCCFF", fg="red", relief=RAISED, bd=3, padx=3, pady=3,
    ...
    def make_main_container_with_node_value_set_and_next_arrow_creation(self, take_notation):
        ...
        self.value_set = Label(self.canvas_make, text=self.value_entry.get(),
                               font=("Arial", 10, "bold"), fg="green", bg="#CCCCFF")
        self.value_set.place(x=self.data_left + 8, y=self.data_up + 3)

        # Node next arrow set
        self.arrow = self.canvas_make.create_line(
            self.data_left+50 + 25, self.data_up + 15, self.data_left+50 + 65, self.data_up + 15, width=4)
        ...
        try:
            ...
            while True:  # For vertical motion in three cases: insert_first, insert_last and insert_after_node
                if take_notation == 1:  # For insert first process
                    self.information.config(
                        text="First node insertion process")
                if self.main_node_up+65 < 320:  # For all function that allowed in that function
                    self.canvas_make.delete(
                        self.main_container_node, self.data, self.next, self.arrow)
                    ...
                    self.main_container_node = self.canvas_make.create_rectangle(
                        self.main_node_left, self.main_node_up, self.main_node_left+100, self.main_node_up+65, outline="brown", width=3)
                    self.data = self.canvas_make.create_rectangle(
                        self.data_left, self.data_up, self.data_left+40, self.data_up+30, outline="green", fill="yellow", width=3)
                    self.next = self.canvas_make.create_rectangle(
                        self.data_left+50, self.data_up, self.data_left+50 + 40, self.data_up+30, outline="green", fill="yellow", width=3)
                    self.next_label.place(
                        x=self.data_label_x+50, y=self.data_label_y)
                    self.data_label.place(
                        x=self.data_label_x, y=self.data_label_y)
                    self.value_set.place(
                        x=self.data_left + 8, y=self.data_up + 3)
                    self.arrow = self.canvas_make.create_line(
                        self.data_left+50 + 25, self.data_up + 15, self.data_left+50 + 65, self.data_up + 15, width=4)
                    self.next_set.place(
                        x=self.data_left+50 + 52, y=self.data_up + 2)
            ...
            if take_notation == 1:  # For insert first
                if len(self.node_value_store) > 0:
                    self.information.config(
                        text="Next part of New node contain the address of start pointing node and \nnow start pointing new node")
                else:
                    self.information.config(
                        text="start pointing New node and next part of new node contains NULL")
            ...
            if len(self.linked_list_data_next_store) > 1 and (take_notation == 0 or take_notation == 2):
                ...
                # temp pointer start from first node
                ...
                while self.temp_label_x < self.linked_list_position[goto][4] + 100 + 20:
                    if take_notation == 2:  # For insert_after_node()
                        ...
                        # otherwise print that information
                        self.information.config(
                            text="Traversing until found the targeting node")

                    else:  # For other function except insert_after_node()
                        self.information.config(
                            text="Traversing until found the last node")

                    # pre location forget
                    self.temp_label.place_forget()
                    self.canvas_make.delete(
                        self.pointing_line_temp, self.temp_pointer)
                    ...
                    self.temp_label.place(
                        x=self.temp_label_x, y=self.temp_label_y)
                    self.pointing_line_temp = self.canvas_make.create_line(self.pointing_line_temp_left,
                                                                           self.pointing_line_temp_up,
                                                                           self.pointing_line_temp_left,
                                                                           self.pointing_line_temp_up + 65, width=2)
            ...
            if len(self.linked_list_data_next_store) > 0:
                ...
                self.linked_list_data_next_store[len(
                ...
                while self.main_node_left < self.linked_list_position[len(self.linked_list_position)-1][4]+100+20:
                    ...
                    if take_notation == 0:  # Makes for insert last only...
                        self.information.config(
                            text="New node added to the last node")
                        time.sleep(0.02)
                        self.window.update()

                    if take_notation == 2:  # No slow motion except insert last
                        self.information.config(
                            text="New node added after the targeting node")
            ...
            # storing widget
            temp_block_number = []
            temp_block_number.append(self.data)
            temp_block_number.append(self.next)
            temp_block_number.append(self.main_container_node)
            self.linked_list_canvas_small_widget.append(temp_block_number)

            # storing widget coordinate
            temp_block_location = []
            temp_block_location.append(self.data_left)
            temp_block_location.append(self.data_up)
            temp_block_location.append(self.data_left+50)
            temp_block_location.append(self.data_up)
            temp_block_location.append(self.main_node_left)
            temp_block_location.append(self.main_node_up)
    ...
    # Reset coordinate and store value,arrow and next label
    def reset_with_store(self, take_notation):
        ...
        if take_notation == 1 and len(self.linked_list_data_next_store) > 1:
            ...
            for i in range(len(self.node_value_store)):
                self.linked_list_data_next_store[i][0].config(
                    text=self.node_value_store[i])
    ...
    def delete_last_node(self, locator):
        # printing all list value :: Create this at the time of creation of the program to checking
        print("linked_list_data_next_store = ",
              self.linked_list_data_next_store)
        print("linked_list_canvas_small_widget",
              self.linked_list_canvas_small_widget)
        print("linked_list_position", self.linked_list_position)
        print("linked_list_canvas_small_widget_label",
              self.linked_list_canvas_small_widget_label)
        print("node_value_store", self.node_value_store)
        ...
        if (locator == 0 or locator == 3) and len(self.linked_list_data_next_store) > 1:
            self.temp_pointer = self.canvas_make.create_rectangle(
                self.temp_pointer_left, self.temp_pointer_up, self.temp_pointer_left + 30, self.temp_pointer_up + 30, fill="red", outline="white", width=3)

            self.temp_label.place(x=self.temp_label_x, y=self.temp_label_y)

            self.pointing_line_temp = self.canvas_make.create_line(
                self.pointing_line_temp_left, self.pointing_line_temp_up, self.pointing_line_temp_left, self.pointing_line_temp_up + 65, fill="white", width=2)
                ...
                while self.temp_label_x < self.linked_list_position[goto][4]+100+20:
                    if locator == 3:  # For delete particular node process
                        if int(self.delete_entry.get()) == 2:
                            ...
                            self.information.config(
                                text="Traversing until found the penultimate node of the targeting node")

                    else:
                        self.information.config(
                            text="Traversing until found the penultimate node of the last node")
            ...
            if locator == 0:  # For last node deletion process
                self.information.config(
                    text="Temp pointing node contains the address that present in the next part of last node and\nand Last node deleted")
            else:
        ...
        if locator == 3:  # For delete particular node process
            ...
            self.temp1_pointer = self.canvas_make.create_rectangle(self.temp_pointer_left+120, self.temp_pointer_up,
                                                                   ...
                                                                   self.temp_pointer_up + 30, fill="red",
                                                                   outline="white", width=3)
            self.temp1_label.place(
                x=self.temp_label_x+120, y=self.temp_label_y)
            self.pointing_line_temp1 = self.canvas_make.create_line(self.pointing_line_temp_left+120,
                                                                    self.pointing_line_temp_up,
                                                                    self.pointing_line_temp_left+120,
                                                                    self.pointing_line_temp_up + 65, fill="white", width=2)

            # status update
            self.information.config(
                text="Temp is pointing the penultimate node of the targeting node and \ntemp1 is pointing the targeting node")
            ...
            for i in range(int(self.delete_entry.get()), len(self.node_value_store)):
                self.linked_list_data_next_store[i -
                                                 1][0].config(text=self.node_value_store[i])
        ...
        if len(self.linked_list_data_next_store) > 0:
            ...
            if len(self.linked_list_data_next_store) > 0:
                temp3 = self.linked_list_position[len(
                    self.linked_list_position)-1]
                self.next_set = Label(self.canvas_make, text="NULL", font=("Arial", 15, "bold"),
                                      fg="#073763", bg="#CCCCFF")
            ...
            print("Node value store(pop)= ", self.node_value_store)
            ...
            if locator == 0 or locator == 3:
                if locator == 3:  # For temp 1 pointer delete process
                    ...
                    self.canvas_make.delete(
                        self.pointing_line_temp1, self.temp1_pointer)
                    self.information.config(
                        text="The next part of the temp is now containing the address that is present in the \nnext part of temp1 and temp1 pointing node is deleted")
            ...
            if len(self.node_value_store) == 0:
                self.information.config(
                    text="List is empty and start pointing NULL")
            elif locator == 0:
    ...
    def delete_first_node(self):
        ...
        # For one node in the list delete process same as delete last
        elif len(self.node_value_store) == 1:
            self.delete_last_node(1)  # delete last function call
            self.information.config(
                text="Now start pointer is containing NULL and first node deleted")
            ...
            self.information.config(
                text="Now start pointer is containing the address that present in the next part of the first node\nand first node deleted")
    ...
    def delete_single_node_infrastructure(self):
        if len(self.node_value_store) == 0:  # Checking if link list is empty
            self.information.config(
                text="Link list is empty  ::  Nothing to delete")
            return
        else:
            self.information.config(text="First node position: 1")
        ...
        self.position_label = Label(self.window, text="Enter the node position you want to delete",
                                    font=("Arial", 13, "bold"), bg="#CCCCFF",
                                    fg="brown")
        self.position_label.place(x=1000, y=620)

        self.position_take_entry = Entry(self.window, font=("Arial", 13, "bold"), bg="#CCCCFF", state=NORMAL,
                                         fg="black", relief=SUNKEN, bd=5, textvar=self.delete_entry)
    ...
    def search_A(self):
        ...
        # input taking process set
        self.position_label = Label(self.window, text="Enter value for search", font=(
            "Arial", 13, "bold"), bg="#CCCCFF", fg="brown")
        self.position_label.place(x=1000, y=620)

        self.position_take_entry = Entry(self.window, font=(
            "Arial", 13, "bold"), bg="#CCCCFF", state=NORMAL, fg="black", relief=SUNKEN, bd=5, textvar=self.delete_entry)
        self.position_take_entry.place(x=1020, y=650)

        self.position_take_entry.focus()

        self.find_btn = Button(self.window, text="Search", font=("Arial", 10, "bold"), bg="#CCCCFF",
                               fg="black", relief=RAISED, bd=3, padx=3, pady=3, state=NORMAL, command=self.show)
        self.find_btn.place(x=1230, y=650)

    def show(self):
        print("linked_list_data_next_store", self.linked_list_data_next_store)
        self.position_label.place_forget()
        self.position_take_entry.place_forget()
        self.find_btn.place_forget()

        self.v = self.position_take_entry.get()
        self.vi = str(self.v)
        self.array = []
        self.array = self.node_value_store.copy()

        # for i in range(0,len(self.node_value_store)):
        #     if self.vi in self.node_value_store[i]:
        #         print("found")
        #         self.information.config(text="found")
        #     else:
        #         print("not found")

        if self.vi in self.array:

            self.pos = self.array.index(self.vi)
            self.pos1 = self.pos+1
            self.information.config(
                text="Value is Found on Position" + str(self.pos1))

        else:
            self.information.config(text="Value is Not Found")
    ...
    #     self.find_btn = Button(self.window, text="Find", font=("Arial", 10, "bold"),
    #                            bg="#CCCCFF", fg="black", relief=RAISED, bd=3, padx=3, pady=3,
    #                            state=NORMAL, command=self.checking_of_existence2)
    #     self.find_btn.place(x=1020, y=650)

    # # Positional node existence checking
    # def checking_of_existence2(self):
    #     self.position_label.place_forget()
    #     self.position_take_entry.place_forget()
    #     self.find_btn.place_forget()
    #     if int(self.position_entry.get()) < 1 or int(self.position_entry.get()) > len(self.node_value_store):
    ...
    #         self.insert_after_node.config(state=DISABLED)
    #         self.information.config(text="Targeting node found")
    #         self.make_node_with_label2(2)

    # # Node making process
    # def make_node_with_label2(self, take_notation):
    #     # Button deactivation
    #     self.insert_at_last.config(state=DISABLED)
    #     self.insert_at_beg.config(state=DISABLED)
    #     self.delete_at_last.config(state=DISABLED)
    ...
    #                                                                  self.main_node_left + 100, self.main_node_up + 65,
    #                                                                  outline="brown", width=3)
    #     self.input_take2(take_notation)

    # # input taking process
    # def input_take2(self, take_notation):
    ...
    # # Working same as function name
    # def make_main_container_with_node_value_set_and_next_arrow_creation2(self, take_notation):
    #     # Deactivate the add btn
    #     self.add_btn.config(state=DISABLED)
    ...
    #     self.insert_node2(take_notation)

    # # Make insert first and insert last within that function
    # def insert_node2(self, take_notation):
    #     try:
    #         self.information.config(text=" ")
    #         self.new_node_label.place_forget()  # 'new node' label release
    #         self.start_initial_point_null.place_forget()  # start pointing NULL release
    ...
    #     self.ev = self.element_take_entry.get()
    #     self.eI = self.position_take_entry.get()

    #     self.eI2 = int(self.eI)

    #     self.node_value_store[self.enI2] = self.ev
