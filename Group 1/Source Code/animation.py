...
class LinkList:  # One and only class of that project
    def __init__(self, root):
        self.window = root
        self.window.config(bg="#8C62AC")

        # Make the Canvas
        self.canvas_make = Canvas(
            self.window, bg="#5E68AC", width=1350, height=500, relief=RAISED, bd=8)
    ...
    def heading_with_label_subheading(self):
        self.head_name = Label(self.window, text="Link List Animation", font=("Arial", 35, "bold", "italic"),
                               bg="#CCCCFF", fg="#073763")
    ...
    def make_start_with_other(self):
        ...
        self.start_pointer = self.canvas_make.create_rectangle(
            self.start_left, self.start_up, self.start_left+30, self.start_up+30, fill="#073763", outline="#CCCCFF", width=3)

        # making of start label
        self.start_label = Label(self.canvas_make, text="start", font=("Arial", 15, "bold"), bg="#CCCCFF",
                                 fg="gray")
        self.start_label.place(x=40, y=410)

        # making of start pointing line
        self.pointing_line_start = self.canvas_make.create_line(
            65, 327, 65, 395, width=2, fill="gray")

        # Making of NULL which is primarily pointing by start
        self.start_initial_point_null = Label(self.canvas_make, text="NULL", font=("Arial", 15, "bold"), bg="#CCCCFF",
                                              fg="blue")
        ...
        self.temp_label = Label(self.canvas_make, text="temp", font=("Arial", 15, "bold"), bg="#CCCCFF",
                                fg="green")

        self.temp1_label = Label(self.canvas_make, text="temp1", font=("Arial", 15, "bold"), bg="#CCCCFF",
                                 fg="green")

    # Make some buttons to give instruction to program
    def make_btn(self):
        self.insert_at_beg = Button(self.window, text="Insert first", bg="#CCCCFF", fg="red",
                                    font=("Arial", 15, "bold"), relief=RAISED, bd=10, command=lambda: self.make_node_with_label(1))
        self.insert_at_beg.place(x=3, y=540)

        self.insert_at_last = Button(self.window, text="Insert last", bg="#CCCCFF", fg="red",
                                     font=("Arial", 15, "bold"), relief=RAISED, bd=10, command=lambda: self.make_node_with_label(0))
        self.insert_at_last.place(x=140, y=540)

        self.delete_at_first = Button(self.window, text="Delete first", bg="#CCCCFF", fg="red",
                                      font=("Arial", 15, "bold"), relief=RAISED, bd=10,
                                      command=self.delete_first_node)
        self.delete_at_first.place(x=280, y=540)

        self.delete_at_last = Button(self.window, text="Delete last", bg="#CCCCFF", fg="red",
                                     font=("Arial", 15, "bold"), relief=RAISED, bd=10,
                                     command=lambda: self.delete_last_node(0))
        self.delete_at_last.place(x=430, y=540)

        self.insert_after_node = Button(self.window, text="insert after ", bg="#CCCCFF", fg="red",
                                        font=("Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL,
                                        command=self.set_of_input_method)
        self.insert_after_node.place(x=580, y=540)

        self.search = Button(self.window, text="Search", bg="#CCCCFF", fg="red",
                             font=("Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL, command=self.search)
        self.search.place(x=740, y=540)

        self.delete_particular_node = Button(self.window, text="Delete particular node", bg="#CCCCFF", fg="red",
                                             font=("Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL,
                                             command=self.delete_single_node_infrastructure)
        self.delete_particular_node.place(x=850, y=540)
        self.updete_node = Button(self.window, text="updete", bg="#CCCCFF", fg="red",
                                  font=("Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL,
                                  command=self.updete)
        self.updete_node.place(x=1100, y=540)

        self.back = Button(self.window, text=" << BACK ", bg="#CCCCFF", fg="red",
                           font=("Arial", 15, "bold"), relief=RAISED, bd=10, state=NORMAL, command=self.window.quit)
        self.back.place(x=1220, y=540)
    ...
    def set_of_input_method(self):
        ...
        self.position_label = Label(self.window, text="Enter the node position after you want to insert new node", font=("Arial", 13, "bold"), bg="orange",
                                    fg="brown")
        self.position_label.place(x=660, y=620)
        ...
        self.position_take_entry.place(x=660, y=650)
        ...
        self.find_btn.place(x=900, y=650)
    ...
    def search(self):
        if len(self.node_value_store) == 0:  # Checking if link list is empty
            self.information.config(
                text="Link list is empty  ::  Nothing to search")
            return
        else:
            self.information.config(text="First node position: 1")
        ...
        # input taking process set
        self.position_label = Label(self.window, text="Enter the Value",
                                    font=("Arial", 13, "bold"), bg="#CCCCFF",
                                    fg="brown")
        self.position_label.place(x=800, y=620)

        self.position_take_entry = Entry(self.window, font=("Arial", 13, "bold"), bg="#CCCCFF", state=NORMAL,
                                         fg="blue", relief=SUNKEN, bd=5, textvar=self.delete_entry)
        self.position_take_entry.place(x=800, y=650)

        self.position_take_entry.focus()

        self.find_btn = Button(self.window, text="Find", font=("Arial", 10, "bold"),
                               bg="#CCCCFF", fg="red", relief=RAISED, bd=3, padx=3, pady=3,
                               state=NORMAL, command=self.search_single_node)
        self.find_btn.place(x=1000, y=650)

    # For search particular node process start
    def search_single_node(self):
        self.position_label.place_forget()
        self.position_take_entry.place_forget()
        self.find_btn.place_forget()

        self.r = self.delete_entry.get()
        if (self.r) in (self.node_value_store):
            # self.s = self.node_value_store.index(self.r)
            messagebox.showerror(
                "Error", self.r+" Value  found ")

        else:
            messagebox.showerror("Error", self.r + " Value not found")
